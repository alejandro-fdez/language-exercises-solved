...
class SlothMachine {
    ...
    play(){
        ...
        const jackpot = this.tryYourLuck();

        this.checkIfANewWinnerIsBorn(jackpot);
    }

    playRoulette(){
        return Boolean(Math.random()>=0.3); // Lo hago así para que salga más a menudo
    }

    tryYourLuck(){
        return this.playRoulette() && this.playRoulette() && this.playRoulette();
    }
    
    checkIfANewWinnerIsBorn(jackpot){
        if(jackpot){
